...
internal class CodeGeneratorVisitor(val context: Context) : IrElementVisitorVoid {
    ...
    */
    ...
    private fun evaluateExplicitArgs(expression: IrMemberAccessExpression): List<LLVMValueRef> {
        ...
        val result = allValueParameters.dropWhile { it !in evaluatedArgs }.map {
            evaluatedArgs[it]!!
        }
        if (!expression.descriptor.isSuspendFunctionOrLambda)
            return result
        val caller = codegen.functionDescriptor!!
        val continuation = if (caller.isSuspend)
            ...
            assert (caller.name.asString() == "doResume"
                    && (caller.containingDeclaration as ClassDescriptor).getSuperClassOrAny() == coroutineImplDescriptor,
                    { "Expected 'CoroutineImpl.doResume' but was '$caller'" })
            codegen.param(caller, 0)                            // Coroutine itself is a continuation.
        }
    ...
    private fun evaluateIntrinsicCall(callee: IrCall, args: List<LLVMValueRef>): LLVMValueRef {
        ...
        when (name) {
            ...
            "konan.internal.getContinuation" -> {
                val caller = codegen.functionDescriptor!!
                assert (caller.isSuspend, { "Expected suspend function but was '$caller'" })
                return codegen.param(caller, caller.allParameters.size)
            }
        }
