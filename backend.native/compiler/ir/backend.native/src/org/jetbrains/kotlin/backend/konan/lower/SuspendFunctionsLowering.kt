...
import org.jetbrains.kotlin.backend.common.descriptors.isSuspendFunctionOrLambda
...
internal class SuspendFunctionsLowering(val context: Context): DeclarationContainerLoweringPass {
        ...
        irDeclarationContainer.declarations.forEach {
            it.acceptChildrenVoid(object: IrElementVisitorVoid {
                ...
                override fun visitCallableReference(expression: IrCallableReference) {
                    ...
                    if (descriptor.isSuspendFunctionOrLambda)
        ...
        irDeclarationContainer.declarations.forEach {
            it.transformChildrenVoid(object: IrElementTransformerVoid() {
                ...
                override fun visitCallableReference(expression: IrCallableReference): IrExpression {
                    ...
                    val descriptor = expression.descriptor
                    if (!descriptor.isSuspendFunctionOrLambda) return expression
    ...
    private fun transformSuspendFunction(irFunction: IrFunction, callableReference: IrCallableReference?): List<IrDeclaration>? {
        ...
        return when (suspendFunctionKind) {
            SuspendFunctionKind.NO_SUSPEND_CALLS -> null                        // No suspend function calls - just an ordinary function.

            SuspendFunctionKind.DELEGATING -> null                              // Calls another suspend function at the end.
                                                                                // No need in state machine.
    ...
    private fun getSuspendFunctionKind(irFunction: IrFunction): SuspendFunctionKind {
        ...
        val statements = when (body) {
            ...
            else -> throw AssertionError("Unexpected body: $body")
        }
        ...
        statements.forEach {
            it.acceptVoid(object: IrElementVisitorVoid {
                ...
                override fun visitCall(expression: IrCall) {
                    ...
                    if (expression.descriptor.isSuspendFunctionOrLambda)
        ...
        val suspendCallAtEnd = lastCall != null && lastCall.descriptor.isSuspendFunctionOrLambda     // Suspend call.
                && (irFunction.descriptor.returnType!!.isValueType()                                 // Does not require
                == (lastCall.descriptor.returnType!!.isValueType()))                                 // boxing/unboxing.
        return when {
    ...
    private fun ClassDescriptor.substitute(types: List<KotlinType>): ClassDescriptor {
        val typeSubstitutor = TypeSubstitutor.create(
                declaredTypeParameters
                        .withIndex()
                        .associateBy({ it.value.typeConstructor }, { TypeProjectionImpl(types[it.index]) })
        )
        return substitute(typeSubstitutor) as ClassDescriptor
    }
    ...
    private abstract class DescriptorWithIrBuilder<out D: DeclarationDescriptor, out B: IrElement>() {

        protected abstract fun buildDescriptor(): D

        protected open fun doInitialize() { }

        protected abstract fun buildBody(): B

        val descriptor by lazy { buildDescriptor() }

        private val builtBody by lazy { buildBody() }
        ...
        val body: B
            get() {
                if (!initialized)
                    throw Error("Access to body before initialization")
                return builtBody
            }
    }
    ...
    private inner class CoroutineBuilder(val irFunction: IrFunction, val callableReference: IrCallableReference?) {
        ...
        fun build(): BuiltCoroutine {
            val superClasses = mutableListOf<ClassDescriptor>(coroutineImplClassDescriptor)
            ...
            if (unboundFunctionParameters != null) {
                ...
                val genericSuspendFunctionClassDescriptor = kotlinPackageScope.getContributedClassifier(
                        Name.identifier("SuspendFunction$numberOfParameters"), NoLookupLocation.FROM_BACKEND) as ClassDescriptor
                suspendFunctionClassDescriptor = genericSuspendFunctionClassDescriptor.substitute(
                        unboundFunctionParameters.map { it.type } + irFunction.descriptor.returnType!!)
                superClasses += suspendFunctionClassDescriptor

                val genericFunctionClassDescriptor = kotlinPackageScope.getContributedClassifier(
                        Name.identifier("Function${numberOfParameters + 1}"), NoLookupLocation.FROM_BACKEND) as ClassDescriptor
                val continuationType = continuationClassDescriptor.substitute(listOf(irFunction.descriptor.returnType!!)).defaultType
                functionClassDescriptor = genericFunctionClassDescriptor.substitute(
                ...
                superClasses += functionClassDescriptor
            }
            coroutineClassDescriptor = ClassDescriptorImpl(
                    ...
                    /* superTypes            = */ superClasses.map { it.defaultType },
            ...
            if (callableReference != null) {
                ...
                val createFunctionDescriptor = coroutineImplClassDescriptor.unsubstitutedMemberScope
                        .getContributedFunctions(Name.identifier("create"), NoLookupLocation.FROM_BACKEND)
                        .singleOrNull { it.valueParameters.size == unboundFunctionParameters!!.size + 1 }
            ...
            coroutineMembers.add(coroutineConstructorBuilder.body)

            coroutineFactoryConstructorBuilder?.let {
                it.initialize()
                coroutineMembers.add(it.body)
            }
            ...
            coroutineMembers.add(doResumeMethodBuilder.body)
        ...
        private fun createConstructorBuilder()
            ...
            override fun buildBody(): IrConstructor {
        ...
        private fun createCreateMethodBuilder(unboundArgs: List<ParameterDescriptor>,
            ...
            override fun buildBody(): IrFunction {
                val startOffset = irFunction.startOffset
                val endOffset = irFunction.endOffset
        ...
        private fun createPropertyGetterBuilder(propertyDescriptor: PropertyDescriptor, type: KotlinType)
            ...
            override fun buildBody() = IrFunctionImpl(
                    startOffset = irFunction.startOffset,
                    endOffset   = irFunction.endOffset,
                    origin      = DECLARATION_ORIGIN_COROUTINE_IMPL,
                    descriptor  = descriptor).apply {
        ...
        private fun createPropertyWithBackingFieldBuilder(name: Name, type: KotlinType, isMutable: Boolean)
            ...
            override fun buildBody(): IrProperty {
                val startOffset = irFunction.startOffset
                val endOffset = irFunction.endOffset
                ...
                return IrPropertyImpl(
                        ...
                        getter       = getterBuilder.body,
                        setter       = setterBuilder?.body)
            }
        }
        ...
        private fun buildPropertyWithBackingField(name: Name, type: KotlinType, isMutable: Boolean): PropertyDescriptor {
            ...
            coroutineMembers.add(propertyBuilder.body)
        ...
        private fun createDoResumeMethodBuilder(doResumeFunctionDescriptor: FunctionDescriptor)
            ...
            override fun buildBody(): IrFunction {
                ...
                suspendResult = LocalVariableDescriptor(
                ...
                return IrFunctionImpl(
                    ...
                    body = irBuilder.irBlockBody(startOffset, endOffset) {
                        ...
                        val statements = when (originalBody) {
                            is IrExpressionBody -> listOf(originalBody.expression)
                            is IrBlockBody -> originalBody.statements
                            else -> throw AssertionError("Unexpected body kind: $originalBody")
                        }
        ...
        private val IrExpression.isSuspendCall: Boolean
            get() = this is IrCall && this.descriptor.isSuspendFunctionOrLambda

        private inner class ExpressionSlicer(val suspensionPointIdType: KotlinType) : IrElementTransformerVoid() {

            override fun visitExpression(expression: IrExpression): IrExpression {
                if (expression.isRootExpression)
                    return processRootExpression(expression)
                return super.visitExpression(expression)
            }

            private fun processRootExpression(expression: IrExpression): IrExpression {
                // Pull up all temporaries to the current level.
                val slicedExpression = sliceExpression(expression)
                return IrBlockImpl(
                        startOffset = expression.startOffset,
                        endOffset   = expression.endOffset,
                        type        = expression.type,
                        origin      = null,
                        statements  = slicedExpression.tempStatements + slicedExpression.expression)
            }

            private inner class SlicedExpression(val expression: IrExpression, val tempStatements: List<IrStatement>)

            private fun sliceExpression(expression: IrExpression): SlicedExpression {
                val irBuilder = context.createIrBuilder(irFunction.descriptor, irFunction.startOffset, irFunction.endOffset)
                irBuilder.run {
                    ...
                    if ((expression is IrWhen && expression.branches.any { it.result.hasSuspendCalls() })
                        ...
                        return SlicedExpression(irGet(tempResult), listOf(irVar(tempResult, expression)))
                    }

                    val tempStatements = mutableListOf<IrStatement>()
                    val slicedChildren = mutableListOf<SlicedExpression>()
                    expression.acceptChildrenVoid(object : IrElementVisitorVoid {
                        ...
                        override fun visitExpression(expression: IrExpression) {
                            slicedChildren.add(sliceExpression(expression))
                        }
                    })
                    val newChildren = slicedChildren.map { it.expression }.toMutableList()
                    if (slicedChildren.any { it.tempStatements.isNotEmpty() }       // Has at least one suspend call inside.
                            || expression.isSuspendCall) {
                        val numberOfChildren = slicedChildren.size

                        val hasSuspendCallInTail = BooleanArray(numberOfChildren + 1)
                        for (i in numberOfChildren - 1 downTo 0)
                            hasSuspendCallInTail[i] = hasSuspendCallInTail[i + 1] || slicedChildren[i].tempStatements.isNotEmpty()

                        for ((index, child) in slicedChildren.withIndex()) {
                            tempStatements += child.tempStatements
                            if ((child.expression.hasSideEffects() && hasSuspendCallInTail[index + 1])
                                    || (expression.isSuspendCall && child.expression.hasExternalVarSets())) {
                                // Save to temporary in order to save execution order.
                                val tmp = IrTemporaryVariableDescriptorImpl(
                                        containingDeclaration = irFunction.descriptor,
                                        name                  = "tmp${tempIndex++}".synthesizedName,
                                        outType               = child.expression.type)
                                tempStatements += irVar(tmp, child.expression)
                    ...
                    if (!expression.isSuspendCall)
                        return SlicedExpression(expression, tempStatements)
                    ...
                    val suspensionPoint = IrSuspensionPointImpl(
                            ...
                            result                     = irBlock {
                                ...
                                +irSetVar(suspendResult, expression)
                                +irReturnIfSuspended()
                                +irGet(suspendResult)
                            },
                            resumeResult               = irBlock {
                                +irCall(restoreStateDescriptor)
                                +irThrowIfGotException()
                                +irGet(dataArgument)
                            })
            ...
            private fun IrElement.hasSideEffects(): Boolean {
                // TODO: more smart criterion.
                var hasCalls = false
            ...
            private val IrExpression.isRootExpression: Boolean
                get() = when (this) {
        ...
        private fun IrBuilderWithScope.irReturnIfSuspended() =
                irIfThen(irEqeqeq(irGet(suspendResult), irGet(COROUTINE_SUSPENDED)),
                        irReturn(irGet(suspendResult)))

        private fun IrBuilderWithScope.irThrowIfGotException() =
                irIfThen(irNot(irEqeqeq(irGet(exceptionArgument), irNull())),
                        irThrow(irImplicitCast(irGet(exceptionArgument), exceptionArgument.type.makeNotNullable())))
    }
    ...
    fun IrBuilderWithScope.irThrow(arg: IrExpression) =
            IrThrowImpl(startOffset, endOffset, arg.type, arg)
